...
void load(char *filename, int N) {
  ...
  for (int n = 0; n < Nelem; n++) {
    ...
    for (int i = 0; i < 4; i++)
      for (int j = 0; j < 4; j++) instream >> surf[n].dbeta[i][j];
...
void doCalculations(int pid) {
  ...
  cout << "calculations for: " << particle->GetName()
       << ", charges = " << baryonCharge << "  " << electricCharge << "  "
       << strangeness << endl;
  ...
  for (int iel = 0; iel < Nelem; iel++) {  // loop over all elements
    ...
    for (int ipt = 0; ipt < pT.size(); ipt++)
      for (int iphi = 0; iphi < phi.size(); iphi++) {
        ...
        double p[4] = {mT, pT[ipt] * cos(phi[iphi]), pT[ipt] * sin(phi[iphi]),
                       0};
        double p_[4] = {mT, -pT[ipt] * cos(phi[iphi]),
                        -pT[ipt] * sin(phi[iphi]), 0};
        double pds = 0., pu = 0.;
        ...
        const double mutot = surf[iel].mub * baryonCharge +
                             surf[iel].muq * electricCharge +
                             surf[iel].mus * strangeness;
          ...
          for (int nu = 0; nu < 4; nu++)
            for (int rh = 0; rh < 4; rh++)
              for (int sg = 0; sg < 4; sg++) {
                ...
                Pi_num[ipt][iphi][mu] += pds * nf * (1. - nf) *
                                         levi(mu, nu, rh, sg) * p_[sg] *
                                         surf[iel].dbeta[nu][rh];
                // computing the extra 'xi' term for the polarization
                for (int ta = 0; ta < 4; ta++)
                  Pi_num_xi[ipt][iphi][mu] +=
                      pds * nf * (1. - nf) * levi(mu, nu, rh, sg) * p_[sg] *
                      p[ta] / p[0] * tvect[nu] *
  ...
  delete[] surf;
  cout << "doCalculations: total, bad = " << setw(12) << Nelem << setw(12)
       << nBadElem << endl;
  cout << "number of elements*pT configurations where nf>1.0: " << nFermiFail
       << endl;
  cout << "event_plane_vectors: " << Qx1 << "  " << Qy1 << "  " << Qx2 << "  "
       << Qy2 << endl;
}
  ...
  for (int iel = 0; iel < Nelem; iel++) {  // loop over all elements
    ...
    for (int mu = 0; mu < 4; mu++)
      for (int nu = 0; nu < 4; nu++) {
        symm_deriv +=
            0.25 *
            pow(hbarC * (surf[iel].dbeta[mu][nu] + surf[iel].dbeta[nu][mu]),
                2) *
            gmumu[mu] * gmumu[nu];
        asymm_deriv +=
            0.25 *
            ...
            gmumu[mu] * gmumu[nu];
        mod_deriv +=
            pow(hbarC * (surf[iel].dbeta[mu][nu]), 2) * gmumu[mu] * gmumu[nu];
        // for(int rh=0; rh<4; rh++)
        // for(int sg=0; sg<4; sg++) {
        //// computing the 'standard' polarization expression
        ...
        //// computing the extra 'xi' term for the polarization
        // for(int ta=0; ta<4; ta++)
        // Pi_num_xi[ipt][iphi][mu] += pds * nf * (1. - nf) * levi(mu, nu, rh,
        // sg)
    ...
    if (fabs(surf[iel].eta) < 0.5) {
      ...
      if (symm_deriv != symm_deriv) cout << "symm_deriv=nan\n";
...
void calcEP1() {
  ...
  cout << "EP1_vectors: " << Qx1 << "  " << Qy1 << "  " << Qx2 << "  " << Qy2
       << endl;
...
void outputPolarization(char *out_file) {
  ...
  for (int ipt = 0; ipt < pT.size(); ipt++)
    for (int iphi = 0; iphi < phi.size(); iphi++) {
      fout << setw(14) << pT[ipt] << setw(14) << phi[iphi] << setw(14)
           << Pi_den[ipt][iphi];
      for (int mu = 0; mu < 4; mu++)
        fout << setw(14)
             << Pi_num[ipt][iphi][mu] * hbarC / (8.0 * particle->GetMass());
      for (int mu = 0; mu < 4; mu++)
        fout << setw(14)
             << -Pi_num_xi[ipt][iphi][mu] * hbarC / (8.0 * particle->GetMass());
      fout << endl;
    }
  fout.close();
  char dim_file[200];
